...
const Home = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    fetch(API_URL + `/api/checksup/${Firebase.auth().currentUser.email}`)
      ...
      .then((json) => {
        if(json.response){
          var current_time = ((Date.now() % 60000) / 1000).toFixed(0)
          if (
            current_time <= json.response.subscriptions.data[0].current_period_end
          ) {
            setsubscribtionStatus(1)
          }
        }
      })
      .catch((error) => {
  ...
  return (
    <ScrollView style={styles.container}>
      <HeaderWelcome
        ...
        userimage={userImage}
      />
      <View style={styles.CarouselContainer}>{/* <HeaderCarousel /> */}</View>
      ...
      {Object.keys(livevideos).length !== 0 && (
        <LiveCarousel
          title="Live Classes"
          data={livevideos}
          nav={navigation}
