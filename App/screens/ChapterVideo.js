...
const ChapterVideo = ({ navigation, route }) => {
  ...
  try {
    route.params.url
      ? (videourl = ADMIN_UPLOADS_URL + JSON.parse(route.params.url)[0].name)
      : null
  } catch {
  ...
  useEffect(() => {
    if (addView == 1) {
      fetch(API_URL + '/api/addview/' + route.params.id).catch((error) => {
        alert('Network Error')
      })
    }
  }, [addView])
    ...
    <ScrollView style={styles.container}>
      {!loaded ? (
        <ActivityIndicator
          size="large"
          color="#00ff00"
          style={{ top: 140, zIndex: 2, padding: 0, marginTop: -36 }}
        />
      ) : (
        false
      )}
          ...
          <View style={styles.tagTextContainer}>
            <View style={styles.tagStyle}>
              <Text style={styles.tagTextStyle}>
                {' '}
                {route.params.subject.toLowerCase()}{' '}
              </Text>
              <Text style={styles.tagSupStyle}> • </Text>
              <Text style={styles.tagTextStyle}>
                {' '}
                {route.params.topic.toLowerCase()}{' '}
              </Text>
            </View>
          </View>
          <Text style={styles.tagDescstyle}>{route.params.description}</Text>
        </View>
      </View>
